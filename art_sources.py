...
def get_session():
    if not hasattr(_thread_local, "session"):
        _thread_local.session = requests_cache.CachedSession(
            "art_api_cache", expire_after=3600
        )
...
def fetch_from_chicago(seen_urls):
    ...
    for _ in range(10):
        ...
        r = session.get(f"{url}?fields={fields}&limit=1&page={page}")
        if not r.ok:
            continue
...
def fetch_from_met(seen_urls):
    ...
    ids_res = session.get(ids_url)
    if not ids_res.ok:
        return None
    all_ids = ids_res.json().get("objectIDs", [])
    if not all_ids:
        return None
...
def fetch_from_cleveland(seen_urls):
    ...
    for _ in range(20):
        page = random.randint(1, 1000)
        r = session.get(f"{base_url}?has_image=1&limit=20&page={page}")
        if not r.ok:
            continue
        data = r.json().get("data", [])
        if not data:
